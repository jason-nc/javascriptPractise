		...
		for(var a = 0; a < blocks[i].length; a++){
			blocks[i][a] = {
						...
						checkCollision: function(ball){
								...
								if (ball.y + ball.vy + ball.radius > this.y && ball.y + ball.vy - ball.radius < this.y + userBlock.height && (ball.x + ball.vx - ball.radius == this.x + this.width || ball.x + ball.vx + ball.radius == this.x)) {
  									ball.vx = -ball.vx;
									return true;
								}
								return false;
			...
			for(var i = 0; i < blocks.length; i++){
				...
				for(var a = 0; a < blocks[i].length; a++){
					blocks[i][a] = {
								...
								checkCollision: function(ball){
										if (ball.y + ball.vy + ball.radius > this.y && ball.y + ball.vy - ball.radius < this.y + this.height && ball.x + ball.vx - ball.radius < this.x + this.width && ball.x + ball.vx + ball.radius > this.x){
											if((ball.y + ball.vy + ball.radius) / this.height >= (ball.x + ball.vx + ball.radius) / this.width){
  												ball.vy = -ball.vy;
											}else{
  												ball.vx = -ball.vx;
											}
